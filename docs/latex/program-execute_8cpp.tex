...
\begin{DoxyCompactList}\small\item\em Verify the size of the context.\+stack vector so that it may be safely accessed. \end{DoxyCompactList}\end{DoxyCompactItemize}
...
{\bfseries Value\+:}
\begin{DoxyCode}{0}
\DoxyCodeLine{  \textcolor{keywordflow}{if} (this-\/>bytecode.size() < (context.pc + (x))) \{ \(\backslash\)}
\DoxyCodeLine{    context.stack.push\_back(GarbageCollected::make<ComputedExpressionError>(\mbox{\hyperlink{classTang_1_1Error}{Error}}\{\textcolor{stringliteral}{"{}Opcode instruction truncated."{}}\})); \(\backslash\)}
\DoxyCodeLine{    context.pc = this-\/>bytecode.size(); \(\backslash\)}
...
{\bfseries Value\+:}
\begin{DoxyCode}{0}
\DoxyCodeLine{  \textcolor{keywordflow}{if} (context.stack.size() < (context.fp + (x))) \{ \(\backslash\)}
\DoxyCodeLine{    context.stack.push\_back(GarbageCollected::make<ComputedExpressionError>(\mbox{\hyperlink{classTang_1_1Error}{Error}}\{\textcolor{stringliteral}{"{}Insufficient context.stack depth."{}}\})); \(\backslash\)}
\DoxyCodeLine{    context.pc = this-\/>bytecode.size(); \(\backslash\)}
\DoxyCodeLine{    break; \(\backslash\)}
\DoxyCodeLine{  \}}
...
\begin{DoxyParams}{Parameters}
{\em x} & The number of entries that should exist in the context.\+stack. \\
\hline
\end{DoxyParams}
